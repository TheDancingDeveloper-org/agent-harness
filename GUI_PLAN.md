...
**Implementation tree:** the current `main` checkout. The implementation was reconciled
from branch `codex/gui-plan` at `deed5a1`; that branch remains provenance, not the status
or implementation source of truth. The browser control plane is implemented in
`src/agent_harness/ui.py`, `query_service.py`,
`browser_session.py`, `templates/`, and `static/`; its in-process journeys are in
`tests/test_ui.py` and `tests/test_ui_packaging.py`. The imported scope includes the GUI
foundation and guarded controls through dependency exploration, adoption, process/gateway
telemetry and reviewed audit operations. The GUI branch's historical build and gate
results are recorded in
[`docs/evidence/2026-08-05-gui-milestone-0-1.md`](docs/evidence/2026-08-05-gui-milestone-0-1.md).

The core changed after this plan was approved: `docs/DESIGN.md` and `docs/STATUS.md` became
the authoritative design/status pair and Stage 1 added a generic, metadata-selected role
runner. The import preserves that runner and its tests. Statements below about the baseline
and milestone state are the plan's 2026-08-05 design record, not present-tense status;
[`docs/STATUS.md`](docs/STATUS.md) §2.8 records the reconciled remainder.
...
The checklist below is the accepted target, not a live progress ledger. In particular,
Milestones 0–4 contain imported slices, but none should be inferred complete from unchecked
prose here; use `docs/STATUS.md` §2.8 for the reconciled implementation and remaining work.
