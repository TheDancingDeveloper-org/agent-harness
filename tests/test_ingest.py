...
store (T22), and a 429 from the pre-classification manifest must never
...
from agent_harness.ingest import (
    SOURCE_CALLS,
    SOURCE_REQUESTS,
    UNCLASSIFIED,
    ingest,
...
def test_a_second_ingest_inserts_nothing(tmp_path: Path) -> None:
    ...
    logs = write_logs(tmp_path)
    store = EventStore(tmp_path / "t.sqlite")
    first = ingest(store, logs)
    second = ingest(store, logs)
...
def test_a_replay_into_a_fresh_store_is_identical(tmp_path: Path) -> None:
    ...
    ingest(a, logs)
    ingest(b, logs)
    ingest(b, logs)  # and again, for good measure
